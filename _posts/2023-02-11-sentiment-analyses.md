...
the Naive Bayes and SVM algorithms were initially used. As they are supervised models, it was
...
- **Removal of special characters, hyperlinks, punctuation, markings users and accents:** do not add value to the construction of the model;
...
SVM classifier. It is possible to see that the main flaw of the model was the false recognition of neutral classes as negative. However, overall the classifier obtained 52% of accuracy.
...
But, as shown on the next graphic, the result of the election vs the predicted on this project shows some differences. The blue bar shows the approval rate on Twitter, and the red bar shows the real result on the election.
